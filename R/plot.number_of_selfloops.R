...
plot_number_of_selfloops <- function(x, ...) {
	...
	if(level == "log") {
		...
		x %>%
			...
			scale_fill_continuous_tableau(palette = "Blue", name = "Number of selfloops") +
			labs(x = "Cases", y = glue("Number of {type} selfloops")) +
			scale_y_continuous() +
			coord_flip() +
			theme_light() +
		...
		if(type == "redo") {
			x %>%
				...
				theme_light() +
				coord_flip() +
				labs(x = "First resource", y = glue("Number of {type} selfloops")) -> p
		}
		else if (type == "repeat") {
			x %>%
				...
				theme_light() +
				coord_flip() +
				labs(x = "Resource", y = glue("Number of {type} selfloops")) -> p
		}
	}
		...
		if(type == "redo") {
			x %>%
				...
				scale_fill_continuous_tableau(glue("Number of {type} selfloops"), palette = "Blue") +
